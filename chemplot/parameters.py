...
def n_neighbors_structural(sample_length):
    prediction = math.exp(predictor(N_INTERCEPT_STRUCTURAL, N_COEFFICIENT_STRUCTURAL, math.log(sample_length)))
    prediction = validator(prediction)
    return prediction
...
def n_neighbors_tailored(sample_length):
    prediction = predictor(N_INTERCEPT_TAILORED, N_COEFFICIENT_TAILORED, math.log(sample_length))
    prediction = validator(prediction)
    return prediction
...
def n_neighbors_structural_pca(sample_length):
    prediction = math.exp(predictor(N_INTERCEPT_STRUCTURAL_PCA, N_COEFFICIENT_STRUCTURAL_PCA, math.log(sample_length)))
    prediction = validator(prediction)
    return prediction
